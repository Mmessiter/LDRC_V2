// LDRC SimRX — Phase 1 USB-HID de-risking spike
// ------------------------------------------------------------------
// Seeed XIAO ESP32-S3, native USB (TinyUSB). Enumerates as a HID
// joystick with NUM_AXES independent 16-bit analog axes and drives each
// axis with an independent sine wave so all channels are individually
// visible in a HID monitor or flight sim.
//
// This is a SPIKE: no RC parsing, no WiFi, no web UI. Just prove that the
// device presents 16 independent analog axes that macOS + neXt can read.
//
...
void setup() {
  ...
  USB.VID(0x1209);
  USB.PID(0x525C);                     // fresh ID so neXt drops the stale
  USB.manufacturerName("LDRC");        // (wrong-order) calibration
  USB.productName("LDRC SimRX");
...
void loop() {
  ...
  if (simrx.ready()) {
    ...
    // ~5 Hz heartbeat blink + log (all channels, for diagnostics).
    if ((int32_t)(now - nextBeat) >= 0) {
      nextBeat = now + 200000UL;
      digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN));
      USBSerial.printf(
        "src=%-5s n=%2u | %4u %4u %4u %4u %4u %4u %4u %4u %4u %4u %4u %4u %4u %4u %4u %4u\n",
        ...
        rcin.channelUs(8),  rcin.channelUs(9),  rcin.channelUs(10), rcin.channelUs(11),
        rcin.channelUs(12), rcin.channelUs(13), rcin.channelUs(14), rcin.channelUs(15));
    }
  }
}
