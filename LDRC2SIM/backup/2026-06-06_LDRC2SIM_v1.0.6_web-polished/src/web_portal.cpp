...
const char* FW_VERSION = "LDRC2SIM-1.0.6";          // parseable: LDRC2SIM-x.y.z
...
// Public = messiter.com — DISABLED for now (empty). Fetching it meant a blocking
// HTTPS/TLS call that 404'd and could hang the single-thread server (felt like a
// crash when leaving the page). Set the URL here once messiter.com is live.
const char* FW_LOCAL_MANIFEST  = "http://m4macmini.local:8001/manifest.json";
const char* FW_PUBLIC_MANIFEST = "";   // e.g. "https://messiter.com/ldrc2sim/manifest.json"
...
.btn-ch{background:#8e6cab}.danger{background:#c97464}
...
void handleStatus() {
  ...
  j += ",\"ch\":[";
  for (uint8_t i = 0; i < 8; i++) { if (i) j += ","; j += String(rc ? rc->channelUs(i) : 1500); }
  j += "]}";
...
void handleChannels() {
  ...
  b += F("</div></div><a class='btn btn-back' href='/'><span class=ico>&#8617;</span>Back</a>"
         "<script>"
         ...
         "if(!stop&&!LDRC.navigating)setTimeout(tick,0);}"  // sample as fast as the round-trip allows
...
void handleUpdateForm() {
  String b = F(
    ...
    "<div id=msg style='margin-top:.3em;font-size:.92em'>Checking for updates&hellip;</div></div>"
...
async function inst(url,name){
 ...
 document.querySelectorAll('.vbtn').forEach(b=>b.disabled=true);$('msg').textContent='Installing '+vr(name)+'…';
 try{const r=await fetch('/api/firmware/install?url='+encodeURIComponent(url),{method:'POST'});
  if(r.ok){$('msg').textContent='✓ Installed '+vr(name)+'. Returning to home in 15s…';setTimeout(()=>location.href='/',15000);}
  else{$('msg').textContent='Failed: '+await r.text();document.querySelectorAll('.vbtn').forEach(b=>b.disabled=false);}}
 catch(e){$('msg').textContent='Network error: '+e;document.querySelectorAll('.vbtn').forEach(b=>b.disabled=false);}}
...
async function chk(){$('msg').textContent='Checking…';$('slocal').style.display='none';$('spublic').style.display='none';
...
void begin(RcInput* rcIn) {
  ...
  if (staConfigured) { WiFi.setAutoReconnect(true); WiFi.begin(cfgSsid.c_str(), cfgPass.c_str()); }
