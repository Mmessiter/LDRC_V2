...
LDRC2SIM src=sine  n= 8  ch1-8us=1500 1500 1500 1500 1500 1500 1500 1500   <- no RX
```
...
**Pages:** **Status** (live protocol + channels, AP/STA state) · **Wi-Fi** (config,
network scan, *Forget*) · **Help** (protocols, recommendations, wiring) · **Update**
(OTA).

**OTA firmware update:** once on your home WiFi, go to `LDRC2SIM.local` → **Update**
→ upload `.pio/build/seeed_xiao_esp32s3/firmware.bin`. The device verifies, flashes
the inactive slot, and reboots; a bad upload is rejected and the current firmware
keeps running.
